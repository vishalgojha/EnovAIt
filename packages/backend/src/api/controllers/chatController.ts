...
type ProviderName = 'Groq' | 'OpenRouter' | 'Ollama' | 'OpenAI Compatible';
...
const FALLBACK = "I'm currently experiencing high demand. Please try again in a moment.";
...
function buildUserFriendlyError(status: number | null, rawMessage: string): { message: string; suggestion: string } {
  if (status === null) {
    ...
    if (rawMessage.toLowerCase().includes('fetch') || rawMessage.toLowerCase().includes('network') || rawMessage.toLowerCase().includes('connect')) {
  ...
  if (status >= 400 && status < 500) {
    return {
      ...
      suggestion: `The server returned error ${status}. This is likely a configuration issue — contact support if it persists.`
    };
  }
...
function buildCandidates(requestedModel?: string): ProviderConfig[] {
...
async function extractFileText(file: FileAttachment): Promise<string> {
...
function convertMcpToolsToOpenAI(tools: McpToolDefinition[]): Array<Record<string, unknown>> {
...
export async function getProvidersHandler(_req: Request, res: Response) {
  ...
  const defaultProvider = providers.find((provider) => provider.id === process.env.AI_PROVIDER)?.id || providers[0]?.id || 'ollama';
